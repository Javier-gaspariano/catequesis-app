...
export default function ExamRunner({ examen }: { examen: Examen }) {
  ...
  async function finalizarEnServidor(forzado = false) {
    let idParaFinalizar = intentoId;

    // si por alguna razón el intento nunca se registró en el servidor (ej. sin conexión al
    // iniciar el examen), intenta registrarlo ahora mismo antes de darse por vencido
    if (!idParaFinalizar && datosAlumno) {
      try {
        const res = await fetch("/api/intentos", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ examenId: examen.id, ...datosAlumno }),
        });
        if (res.ok) {
          const nuevoIntento = await res.json();
          idParaFinalizar = nuevoIntento.id;
          setIntentoId(nuevoIntento.id);
          // reenvía todas las respuestas ya contestadas localmente
          for (const [preguntaId, valor] of Object.entries(respuestas)) {
            await fetch(`/api/intentos/${nuevoIntento.id}/respuestas`, {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ preguntaId, valor }),
            }).catch(() => {});
        ...
        // sigue sin conexión: se maneja abajo
      }
    }

    if (!idParaFinalizar) {
      // de verdad no hay forma de conectar: se guarda todo local y se marca como pendiente de sincronizar
      if (!forzado) {
        setErrorEnvio(
          "No se pudo conectar para calificar tu examen. Tus respuestas están guardadas en este dispositivo — revisa tu conexión a internet e inténtalo de nuevo."
        );
      }
      setFase("finalizado");
      return;
    ...
    setErrorEnvio(null);
    try {
      const res = await fetch(`/api/intentos/${idParaFinalizar}/finalizar`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.certificado) setCertificadoUrl(`/api/certificados/${idParaFinalizar}`);
      setFase("finalizado");
    } catch {
      if (!forzado) {
  ...
  if (fase === "finalizado") {
    return (
      ...
      >
        ...
        {errorEnvio && (
          <div className="mt-4 flex max-w-sm flex-col items-center gap-3 rounded-2xl bg-girasol/10 px-4 py-3">
            <p className="text-sm font-semibold text-girasol">{errorEnvio}</p>
            {!resultadoFinal && (
              <button
                onClick={() => finalizarEnServidor(false)}
                disabled={enviando}
                className="rounded-full bg-girasol px-6 py-2 font-display text-sm font-extrabold text-white disabled:opacity-50"
              >
                {enviando ? "Reintentando..." : "🔄 Reintentar calificar"}
              </button>
            )}
          </div>
        )}
