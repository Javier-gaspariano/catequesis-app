...
export default function Estadisticas() {
  ...
  const [filtroExamenSeg, setFiltroExamenSeg] = useState("");
  const [filtroSacramentoSeg, setFiltroSacramentoSeg] = useState("");
  ...
  const [capillas, setCapillas] = useState<CapillaResumen[]>([]);
  const [lecturaMensaje, setLecturaMensaje] = useState<LecturaMensaje[]>([]);
  const [alumnosPorCatequista, setAlumnosPorCatequista] = useState<{ catequista: string; alumnos: number }[]>([]);
  ...
  useEffect(() => {
    ...
    fetch("/api/capillas")
      .then((r) => r.json())
      .then((d) => setCapillas(Array.isArray(d) ? d : []));
  }, []);
  ...
  useEffect(() => {
    ...
    fetch(`/api/estadisticas/alumnos-por-catequista?${params.toString()}`)
      .then((r) => r.json())
      .then(setAlumnosPorCatequista);
  }, [fechaDesde, fechaHasta]);

  useEffect(() => {
    const params = new URLSearchParams({ por: dimension });
    if (filtroExamenSeg) params.set("examenId", filtroExamenSeg);
    if (filtroSacramentoSeg) params.set("sacramento", filtroSacramentoSeg);
    fetch(`/api/estadisticas/segmentacion?${params.toString()}`).then((r) => r.json()).then(setSegmentos);
  }, [dimension, filtroExamenSeg, filtroSacramentoSeg]);

  useEffect(() => {
    const params = new URLSearchParams();
    if (filtroExamen) params.set("examenId", filtroExamen);
    if (filtroCatequista) params.set("catequistaId", filtroCatequista);
    if (filtroCapilla) params.set("capillaId", filtroCapilla);
  ...
  }, [filtroExamen, filtroCatequista, filtroCapilla]);
    ...
    <div className="flex flex-col gap-8">
      ...
      <section className="rounded-2xl bg-white p-5 shadow-sm">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            <select value={filtroExamenSeg} onChange={(e) => setFiltroExamenSeg(e.target.value)} className={campo}>
            ...
            <select value={filtroSacramentoSeg} onChange={(e) => setFiltroSacramentoSeg(e.target.value)} className={campo}>
      ...
      <section className="rounded-2xl bg-white p-5 shadow-sm">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <h2 className="font-display text-lg font-bold text-tinta">Alumnos por catequista</h2>
          <div className="flex flex-wrap items-center gap-2">
            <label className="flex items-center gap-1 text-xs font-semibold text-tinta-suave">
              ...
              <input
                ...
                className="rounded-full border-2 border-cielo/15 px-2 py-1 text-xs outline-none focus:border-cielo"
              />
            </label>
            <label className="flex items-center gap-1 text-xs font-semibold text-tinta-suave">
              Hasta
            ...
            {(fechaDesde || fechaHasta) && (
              <button
                ...
                className="text-xs font-bold text-girasol"
              >
                Limpiar
              </button>
            )}
        ...
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
            <Pie data={alumnosPorCatequista} dataKey="alumnos" nameKey="catequista" outerRadius={110} label>
              {alumnosPorCatequista.map((_, i) => (
        ...
        {alumnosPorCatequista.length === 0 && (
          <p className="text-center text-sm text-tinta-suave">No hay datos con este rango de fechas.</p>
        )}
      </section>
      ...
      <section className="rounded-2xl bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <a
              href={`/admin/estadisticas/imprimir?${new URLSearchParams({
                ...(filtroExamen && { examenId: filtroExamen }),
                ...(filtroCatequista && { catequistaId: filtroCatequista }),
                ...(filtroCapilla && { capillaId: filtroCapilla }),
                ...(filtroDificultad && { dificultad: filtroDificultad }),
                ...(filtroPorcentajeMin && { porcentajeMin: filtroPorcentajeMin }),
                ...(filtroPorcentajeMax && { porcentajeMax: filtroPorcentajeMax }),
              }).toString()}`}
              target="_blank"
              rel="noopener noreferrer"
              className="rounded-full bg-sol/20 px-4 py-2 text-sm font-bold text-tinta hover:bg-sol/30"
            >
              🖨️ Reporte imprimible
            </a>
            <button
            ...
            return (
              <div
                ...
                className={`flex items-center justify-between rounded-xl border p-3 ${
                  debil ? "border-2 border-girasol bg-girasol/10" : "border-cielo/10"
                }`}
              >
                <span className="max-w-md truncate text-sm">
                  {p.enunciado}
                  {debil && <span className="ml-2 font-bold text-girasol">⚠ Reforzar este tema</span>}
                </span>
                <div className="flex items-center gap-3 text-xs font-bold">
                  <span
                    className={`rounded-full px-2 py-1 ${debil ? "bg-girasol/20 text-girasol" : "bg-cielo/10 text-cielo"}`}
                  >
                    {p.porcentajeAciertos}% aciertos
                  </span>
